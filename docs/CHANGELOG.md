...
| 13 Sep | **`calibration.json` re-measured on four factors.** It was still the 31 Aug five-factor measurement. `backtest.py` now drops the unfinished current month (running on the 13th had scored August against a 13-day September). 83 complete months, 7,319 observations: **flat at 47.3%, no low-to-high trend (Cochran–Armitage p = 0.53)**. The old "top decile is the lowest (44.2%)" was noise — now 47.0%. The site's trust label reads the month count from the data instead of hard-coding "84"; `onboard.html` no longer lists `p_win` as unbuilt. | this entry |
